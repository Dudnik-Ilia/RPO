...
import com.rpo.spring_back.tools.DataValidationException;
...
public class CountryController {
    ...
    // GET запрос для вывода списка разрабов для страны
    @GetMapping("/countries/{id}/artists")
    public ResponseEntity<List<Developer>> getCountryArtists(@PathVariable(value = "id") Long countryId) {
        Optional<Country> cc = countryRepository.findById(countryId);
        if(cc.isPresent()) {
            return ResponseEntity.ok(cc.get().developers);
        }
        return ResponseEntity.ok(new ArrayList<Developer>());
    }
}
